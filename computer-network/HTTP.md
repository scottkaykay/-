...
· HTTP分块传输编码允许服务器为动态生成的内容维持HTTP持久连接，因为动态生成的内容是无法确定Content-Length的;\
· 允许最后发送Content-Length头消息字段，适用于内容被确定之前无法确定Content-Length的情况。\
· 有利于一边压缩一边发送数据，而不是先完成压缩过程以得知压缩后数据的大小。
