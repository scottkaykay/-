...
## 文件存储了10亿条销量数据，如何找出前1000大的数据？

·方法一：排序，时间复杂度O(NlogN), 效率较低，只取前面一小部分数据，排序是对整体排序，做了很多无用功\
·方法二：局部淘汰法：与排序类似，用一个容器保存前1000个数，然后将所有剩余的数字与容器里的最小数字相比，如果所有后续的元素都比容器中的最小元素小，则容器中的数为最大的1000个数。
