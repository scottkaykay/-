...
· new可以重载，malloc不允许重载\
· new从自由存储区上为对象动态分配内存，malloc从堆上申请内存；\
· 对于自定义类型，new会先调用operate new函数申请足够的内存，然后调用类型的构造函数进行初始化，最后返回自定义类型指针。delete先调用析构函数，
然后调用operate delete函数释放内存，

new底层动作：\
· 对于内置简单数据类型，直接调用operator new分配内存;\
· 对于复杂数据类型，如自定义类，先调用operator new分配内存，然后在分配的内存上调用构造函数初始化。\
· 如果new失败，可以使用new_handler来处理: \_set_new_handler(函数名)\
· new[], 比如new[3],先调用operator new 分配内存，然后在前4个字节写入数组大小，然后调用3次构造函数

delete:\
· 对于简单类型，直接调用free函数释放\
· 对于复杂类型，先调用析构函数，再调用operator delete

malloc的实质就是将多个可用的内存块链接成所谓的空闲列表，然后在这个列表中找到符合要求的大小足够的内存块，将其一分为二，其中一份大小为用户申请的大小，另一份返回到列表中，用户申请的内存块使用free释放后，返回到空闲列表，多次分配与释放后，内存块越来越碎片化，此时需要对其进行整理合并，得到更大的内存块以满足要求。
